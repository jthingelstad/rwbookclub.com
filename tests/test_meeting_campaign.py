...
def _member(slug, *, attendance="pending", reading="unknown", email=True,
            last_asked_days=None, attendance_asks=0, reading_asks=0, progress=None):
    last_asked = None if last_asked_days is None else (TODAY - timedelta(days=last_asked_days)).isoformat()
    return {
        "member": slug.title(), "memberSlug": slug, "memberId": 1,
        "attendance": attendance, "reading": reading, "readingProgress": progress,
        "emailLinked": email, "lastAskedAt": last_asked,
        "attendanceAsks": attendance_asks, "readingCheckinCount": reading_asks,
    }
...
def test_attendance_needed_for_pending_and_unsure():
    got = _by_slug(_plan(10, _member("a", attendance="pending", last_asked_days=4),
...
def test_already_engaged_member_is_not_force_pinged():
    ...
    c = _by_slug(_plan(8, _member("a", attendance="yes", reading="on_track", last_asked_days=None)))["a"]
...
def test_give_up_on_a_silent_member():
    ...
    assert _by_slug(_plan(8, _member("a", attendance="pending",
                                     attendance_asks=2, last_asked_days=4)))["a"]["kind"] == "attendance"
...
def test_give_up_only_applies_to_the_silent():
    ...
    c = _by_slug(_plan(8, _member("a", attendance="yes", reading="on_track",
                                  reading_asks=5, last_asked_days=4)))["a"]
...
def test_no_reading_checkin_cap():
    ...
    c = _by_slug(_plan(8, _member("a", attendance="yes", reading="behind",
                                  last_asked_days=4, reading_asks=9)))["a"]
    assert c["kind"] == "reading" and c["asksSoFar"] == 9 and c["mustReach"] is False


def test_asks_so_far_comes_from_the_right_counter():
    att = _by_slug(_plan(10, _member("a", attendance="pending",
                                     last_asked_days=4, attendance_asks=2)))["a"]
    assert att["asksSoFar"] == 2
    rd = _by_slug(_plan(10, _member("b", attendance="yes", reading="behind",
                                    last_asked_days=4, reading_asks=3)))["b"]
