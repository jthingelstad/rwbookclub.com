...
def _ol_doc(**over):
    doc = {"title": "Nexus", "author_name": ["Yuval Noah Harari"], "first_publish_year": 2024,
           "number_of_pages_median": 528, "subject": ["Artificial intelligence", "History"],
           "ratings_average": 3.9, "ratings_count": 1200, "key": "/works/OLX"}
    doc.update(over)
    return doc
...
def test_pick_fit_already_read_headline(fresh_db, monkeypatch):
    from agent.enrich import openlibrary as enrich_ol
    monkeypatch.setattr(enrich_ol, "search_best_match",
                        lambda t, a: (_ for _ in ()).throw(AssertionError("no OL for corpus hit")))
    out = json.loads(dispatch("pick_fit", {"title": "Watchmen"},
                              {"member_slug": "erik", "channel_id": "123"}))
...
def test_pick_fit_unresolved_degrades(fresh_db, monkeypatch):
    ...
    out = json.loads(dispatch("pick_fit", {"title": "A Book Nobody Indexed"},
...
def test_pick_prospects_defaults_to_asker_and_splits_cloud(fresh_db):
    db.add_memory("Prefers translated fiction", scope="member", subject="jamie", source="reflection")
    db.add_book_cloud_entry(title="The Power Broker", reason="Jamie floated it as a systems book",
                            surface="discord", mentioned_by="jamie")
    db.add_book_cloud_entry(title="Piranesi", reason="Loren suggested as light fiction",
                            surface="discord", mentioned_by="loren")
    db.add_book_cloud_entry(title="Watchmen", reason="already-read mention", surface="discord",
                            book_slug="watchmen", mentioned_by="tom")
