...
def test_roll_call_subject():
    assert meeting_rules.roll_call_subject(STATUS) == "Roll call: A World Appears on Tuesday, June 30"


def test_roll_call_subject_falls_back_without_book():
    status = {"meeting": {"date": "2026-06-30", "book": None}, "counts": STATUS["counts"]}
    assert meeting_rules.roll_call_subject(status) == "Roll call: the next meeting on Tuesday, June 30"
...
def test_roll_call_email_body_includes_time_and_location_when_set():
    status = {
        "meeting": {"date": "2026-07-28", "startTime": "18:30", "location": "Broder's",
                    "book": {"title": "Stiff"}, "pickerNames": ["Tom"]},
