...
def _log_turns(slug: str, n: int, channel: str = "999"):
    for i in range(n):
        db.log_message(channel, "user", f"turn {i} about books", speaker=slug.title(),
                       member_slug=slug)
...
def test_adds_land_with_reflection_source(fresh_db, monkeypatch):
    ...
    assert {m["note"] for m in mems} >= {"Prefers translated fiction", "DNFs dense specialist slogs"}
...
def test_provenance_enforced_in_code(fresh_db, monkeypatch):
    ...
    protected = db.add_memory("Jamie asked me to remember his picks alternate fiction/nonfiction",
                              scope="member", subject="jamie", source="Jamie")
    owned = db.add_memory("Old reflection note", scope="member", subject="jamie",
                          source=reflection.SOURCE)
    _log_turns("jamie", 4)
    monkeypatch.setattr(reflection.oliver, "complete", lambda *a, **k: json.dumps(
        {"add": [], "update": [{"id": protected, "note": "OVERWRITTEN"}],
...
def test_unparseable_output_skips_member_and_keeps_watermark(fresh_db, monkeypatch):
    _log_turns("jamie", 4)
    monkeypatch.setattr(reflection.oliver, "complete",
                        lambda *a, **k: "Sorry, I could not decide.")
    out = reflection.run()
...
def test_first_run_mail_cursor_is_forward_only(fresh_db, monkeypatch):
    ...
    db.upsert_mail_message({
        "message_id": "old-1", "thread_id": "t-0", "from_email": "jthingelstad@gmail.com",
        "from_name": "Jamie", "member_slug": "jamie", "subject": "Ancient",
        "sent_at": "2020-01-01T00:00:00Z", "received_at": "2020-01-01T00:00:00Z",
        "body_text": "old", "body_clean": "old",
    })
    called = []
    monkeypatch.setattr(reflection.oliver, "complete", lambda *a, **k: called.append(1) or "{}")
    reflection.run()
...
def test_dry_run_writes_nothing(fresh_db, monkeypatch, capsys):
    _log_turns("jamie", 4)
    monkeypatch.setattr(reflection.oliver, "complete", lambda *a, **k: json.dumps(
        {"add": ["A durable note"], "update": [], "retire": []}))
