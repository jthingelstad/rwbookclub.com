    ...
    def test_author_bio_coalesce(self):
        with db.connect() as conn:
            ...
            clubdb.upsert_author_enrichment(
                conn, aid, {"bio": "enriched bio", "nationality": "Canada",
...
class TestEnrichBook:
    def test_writes_sidecar_from_sources(self, monkeypatch):
        monkeypatch.setattr(ol, "book_facts", lambda *a, **k: {
            "ol_key": "/works/OLX", "ol_cover_id": 123, "edition_count": 5,
            "languages": ["eng"], "subjects": ["Space"], "synopsis": "syn",
            "publication_year": 2011, "page_count": 300, "isbn13": "9780000000000",
            "ratings_average": 4.2, "ratings_count": 50, "author_keys": [],
        })
        monkeypatch.setattr(wd, "resolve_book", lambda *a, **k: {"id": "Q1"})
        monkeypatch.setattr(wd, "book_facts", lambda ent: {
            "wikidata_id": "Q1", "awards": ["Hugo Award"], "series": "A Series",
            "goodreads_id": "g1", "wikipedia_url": "https://wiki/x"})
        with db.connect() as conn:
    ...
    def test_curated_synopsis_not_mirrored(self, monkeypatch):
        monkeypatch.setattr(ol, "book_facts", lambda *a, **k: {
            "synopsis": "OL synopsis", "author_keys": []})
...
class TestEnrichAuthor:
    def test_bio_gapfills_from_wikipedia_and_merges(self, monkeypatch):
        ...
        monkeypatch.setattr(wd, "author_facts", lambda ent: {
            ...
            "image_filename": None, "wikipedia_url": "https://wiki/a",
            "wikipedia_title": "Author A"})
        monkeypatch.setattr(wp, "summary", lambda *a, **k: {
            "extract": "Wikipedia bio", "thumbnail_url": None,
...
class TestCorpusGenEmitsEnrichment:
    BASE_BOOK = {
        "id": 1, "title": "T", "subtitle": None, "author_names": ["A"],
        "topic": "X", "fiction": 1, "publication_year": 2000, "page_count": 100,
        "isbn13": None, "ol_key": None, "synopsis": None, "picker_slugs": [],
        "subjects_json": None, "subjects": [],
    }

    def test_book_doc_emits_present_fields_only(self):
        b = {**self.BASE_BOOK, "edition_count": 7, "languages": ["eng"],
             "ratings_average": 4.5, "ratings_count": 12, "series": "S",
             "awards": ["Hugo"], "wikidata_id": "Q9",
             "wikipedia_url": "https://w", "goodreads_id": "g"}
    ...
    def test_book_doc_omits_empty_enrichment(self):
        b = {**self.BASE_BOOK, "edition_count": None, "languages": [],
             "ratings_average": None, "ratings_count": None, "series": None,
             "awards": [], "wikidata_id": None, "wikipedia_url": None,
             "goodreads_id": None}
        doc = corpus_gen._book_doc(b)
        for absent in ("editionCount", "languages", "ratingsAverage", "awards",
                       "wikidataId", "wikipediaUrl", "goodreadsId", "series"):
            assert absent not in doc

    def test_author_doc_emits_enrichment(self):
        a = {"name": "A", "bio": "b", "birth_year": 1900, "death_year": 1980,
             "nationality": "France", "website": "https://s",
             "wikipedia_url": "https://w", "notable_works": ["W1"],
    ...
    def test_author_doc_omits_empty(self):
        a = {"name": "A", "bio": None, "birth_year": None, "death_year": None,
             "nationality": None, "website": None, "wikipedia_url": None,
             "notable_works": [], "photo_credit": None}
