...
def test_log_message_tags_member_and_search_filters(fresh_db):
    db.log_message(
        "email:t1", "user", "Which sci-fi should we read?", speaker="Jamie", member_slug="jamie"
    )
...
def test_recent_threads_for_member_surfaces_other_medium(fresh_db):
    ...
    db.log_message(
        "email:t1",
        "assistant",
        "Superintelligence, The Coming Wave, The Dawn of Everything",
        member_slug="jamie",
    )
...
def test_outbound_email_archived_two_sided(fresh_db):
    ...
    mail_archive.archive_outbound_email(
        inbound,
        body="Try Blindsight and Nexus.",
        to_emails=["jthingelstad@gmail.com"],
...
def test_backfill_resolves_speaker_names(fresh_db):
    ...
    db.log_message(
        "email:t9", "user", "hello there", speaker="Jamie Thingelstad"
    )  # full display name
