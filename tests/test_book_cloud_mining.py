...
def _mail(mid: str, slug: str, sent_at: str, body: str):
    db.upsert_mail_message({
        "message_id": mid, "thread_id": f"t-{mid}", "from_email": f"{slug}@x.com",
        "from_name": slug.title(), "member_slug": slug, "subject": "Books",
        "sent_at": sent_at, "received_at": sent_at, "body_text": body, "body_clean": body,
    })
...
def test_mentions_inserted_backdated_with_provenance(fresh_db, monkeypatch):
    ...
    monkeypatch.setattr(miner.oliver, "complete", lambda *a, **k: json.dumps([
        {"title": "The Power Broker", "author": "Robert Caro",
         "reason": "nominated as a power-and-institutions pick", "reason_kind": "nomination",
         "message_id": "m1"}]))
...
def test_unparseable_after_retry_returns_none(fresh_db, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(miner.oliver, "complete",
                        lambda *a, **k: calls.append(1) or "I can't decide.")
...
def test_empty_year_costs_nothing(fresh_db, monkeypatch):
    ...
    assert miner._mine_member_year("tom", 2019, until="2026-01-01",
                                   dry_run=False) == {"mentions": 0, "messages": 0}
