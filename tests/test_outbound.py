...
def test_send_multipart_text_and_html(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        outbound.signature, "email_signatures", lambda: ("SIG", '<div class="oliver-sig">SIG</div>')
    )
    monkeypatch.setattr(
        outbound.email_jmap, "send_email", lambda **kw: captured.update(kw) or {"emailId": "e1"}
    )
...
def test_linked_member_policy_is_checked_before_delivery(fresh_db, monkeypatch):
    ...
    with pytest.raises(ValueError, match="linked member"):
        outbound.send(
            ...
            idempotency_key="email:policy-denied",
        )
    assert calls == []
    ...
    outbound.send(
        ...
        body="Hi",
        policy="linked_member",
        idempotency_key="email:policy-allowed",
    )
    assert len(calls) == 1
