...
class TestDispatchHappyPaths:
    ...
    def test_compare_books_returns_side_by_side(self):
        ...
        result = json.loads(
            dispatch("compare_books", {"books": ["the-martian", "thinking-in-systems"]}, {})
        )
    ...
    def test_record_availability_saves_for_speaker(self, fresh_db):
        ...
        assert result["saved"] is True
        rows = fresh_db.meeting_member_status_for_meeting(
            result["meetingStatus"]["meeting"]["meetingId"]
        )
    ...
    def test_send_email_blocks_unknown_recipient(self, monkeypatch, fresh_db):
        ...
        assert result == {
            "error": "Oliver can only email linked book club member addresses from this tool"
        }
    ...
    def test_request_reading_update_blocked_inside_inbound_email_channel(
    ...
    def test_request_roll_call_update_sends_to_linked_current_members(self, monkeypatch, fresh_db):
        ...
        assert all(
            "Roll call: A World Appears on Tuesday, June 30" == call["subject"] for call in sent
        )
    ...
    def test_request_roll_call_update_all_skipped_does_not_open_roll_call(
        self, monkeypatch, fresh_db
    ):
        from agent import config
        from agent.club import meeting_rules
        ...
        meeting = meeting_rules.next_meeting()
        fresh_db.record_attendance_report(
            meeting["meetingId"], clubdb.lookup_member_id("jamie"), "yes"
        )
        identities.link_member_email("jamie@thingelstad.com", "jamie")
        result = json.loads(
            dispatch(
    ...
    def test_request_roll_call_update_blocked_inside_inbound_email_channel(
        self, monkeypatch, fresh_db
    ):
        from agent.mail import email_jmap
        from agent.tools import dispatch
    ...
    def test_meeting_readiness_combines_attendance_and_reading(self, fresh_db):
        ...
        fresh_db.record_reading_report(
            meeting["meetingId"], clubdb.lookup_member_id("jamie"), "finished"
        )
    ...
    def test_meeting_readiness_requires_picker_attendance(self, fresh_db):
        ...
        non_pickers = [
            m for m in corpus_read.members() if m.get("isCurrent") and m["slug"] not in pickers
        ]
    ...
    def test_recent_channel_context_returns_logged_messages(self, fresh_db):
        ...
        result = json.loads(
            dispatch("recent_channel_context", {"limit": 5}, {**JAMIE_CTX, "channel_id": "ch1"})
        )
    ...
    def test_search_discussion_clamps_limit(self, fresh_db):
        ...
        result = json.loads(
            dispatch("search_discussion", {"query": "borges", "limit": 99}, JAMIE_CTX)
        )
        ...
        saved = json.loads(
            dispatch(
                "record_timeline_event",
                {
                    ...
                    "kind": "dinner",
                    "date": "2019-05-01",
                    "summary": "The club had dinner at Nick's.",
                    "member": "nick",
                },
                NICK_CTX,
            )
    ...
    def test_record_timeline_event_rejects_bad_kind_for_category(self, fresh_db):
        ...
        result = json.loads(
            dispatch(
                ...
                {
                    "category": "social",
                    "kind": "book_picked",  # book_picked is a selection kind
                    "date": "2019-05-01",
