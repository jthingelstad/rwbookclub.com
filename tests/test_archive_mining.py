...
def test_mail_messages_between_bounds(fresh_db):
    ...
    rows = db.mail_messages_between(
        "2018-01-01", "2019-01-01", member_slug="jamie", exclude_from="oliver@rwbookclub.com"
    )
    assert [r["message_id"] for r in rows] == [
        "a"
    ]  # 2019 out of range; tom filtered; oliver excluded
    both = db.mail_messages_between(
        "2018-01-01", "2019-01-01", exclude_from="oliver@rwbookclub.com"
    )
...
def test_lane_stops_on_failure_and_resumes_there(fresh_db, monkeypatch):
    _mail("m18", "tom", "t@x.com", "2018-03-01T00:00:00Z")
    _mail("m19", "tom", "t@x.com", "2019-03-01T00:00:00Z")
    monkeypatch.setattr(miner.reflection, "consolidate", lambda *a, **k: {"skipped": "unparseable"})
...
def test_dry_run_leaves_no_cursor(fresh_db, monkeypatch):
    _mail("m18", "tom", "t@x.com", "2018-03-01T00:00:00Z")
    monkeypatch.setattr(
        miner.reflection, "consolidate", lambda *a, **k: {"add": 1, "update": 0, "retire": 0}
    )
