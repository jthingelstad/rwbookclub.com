...
def test_create_member_stamps_today():
    with db.connect() as conn:
        ...
        joined = conn.execute(
            "SELECT joined FROM club_members WHERE id=?", (res["id"],)
        ).fetchone()[0]
...
def test_tenure_filter_on_grid_rows():
    ...
    rows = [
        {"slug": "old", "date": "2005-01-01"},
        {"slug": "new", "date": "2020-01-01"},
...
def test_stale_review_drafts_expire_and_release_member(fresh_db):
    ...
    with db.connect() as conn:  # age it past the expiry window
        conn.execute(
            "UPDATE review_drafts SET created_at = datetime('now','-30 days') WHERE id=?", (did,)
        )
