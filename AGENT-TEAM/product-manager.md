...
1. Run the git preflight (`AGENT-TEAM/scripts/preflight.sh`). A dispatcher-created project thread
   names one issue and arrives with the dispatcher's `wip` claim; accept it as yours rather than
   skipping it.
...
8. On a dispatched run, remove `dispatch:product` and `wip`. If the result is new direction,
...
10. Drop a `notes/` run log (`AGENT-TEAM/scripts/new-note.sh product-manager <slug>`). End with `git status` clean.
