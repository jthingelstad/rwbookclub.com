...
2. Identify the cultural question, member signal, or conversation norm at stake. An issue-driven
   project thread names the issue and arrives with the initiating conversation's `wip` claim;
...
7. Complete an issue-driven handoff by removing `dispatch:culture`, `needs-culture`, and `wip`. If a
