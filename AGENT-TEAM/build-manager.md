...
2. Pick **exactly one** issue. A dispatcher-created project thread names the issue and arrives with
   the dispatcher's `wip` claim; accept that claim as yours rather than skipping it. Otherwise skip
   existing `wip` and
...
3. If a manual thread started without a claim, add `wip` before working. Keep the claim until the
