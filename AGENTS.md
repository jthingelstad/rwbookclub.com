...
The canonical, maintained guide for this repo: architecture, schema, build/deploy, gotchas, and
the full "things not to do". `CLAUDE.md` is a symlink to this file, so Claude Code and Codex read
the same instructions; do not fork them.

Project-specific context for coding agents. The site is live at rwbookclub.com (served by GitHub Pages from the **`gh-pages` branch**, built + deployed **locally** by Oliver — not from `main`/CI). Club counts change over time; query the `club_*` tables or generated corpus instead of copying a point-in-time count into code or prompts.
...
- **`agent/`** — Oliver, the club's Discord bot (Python). Consumes the corpus; answers questions in `#ask-oliver` via Claude. Structured member pronouns live in the Class-B/private `member_preferences` table, keyed to `club_members.id`; they are injected only into Oliver's private context as a quiet grammar aid and are never projected into the corpus or website. Members self-manage the optional value in `/oliver my-club`, and a missing value means Oliver uses the member's name or singular they/them rather than inferring.
