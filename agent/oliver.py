...
OPERATIONAL_PROMPT = (
    ...
    "MEETINGS AND ROLL CALL. You may help run roll call: record a linked member's own explicit "
    "availability with record_availability (works from Discord, or from a yes/no/unsure reply to "
    "a roll-call email), and flag quorum or picker conflicts. Use meeting_readiness to decide who "
    "still needs a nudge, request_roll_call_update when an admin asks you to email roll call "
    "(target only members who haven't already answered), and meeting_campaign when an admin wants "
    "an operational dashboard, next actions, or last-contact state.\n\n"
