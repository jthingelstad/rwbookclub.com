...
Entity-relationship reference for `agent/oliver.db` (SQLite). Originally generated by introspecting
the live schema on 2026-06-27; the 6 `mail_message_fts*` tables are one FTS5 virtual table + its
shadows, shown here as one node.

The database holds **two classes** of data (see `AGENTS.md` for the maintained architecture):
...
erDiagram
    ...
    club_lists          ||--o{ club_list_books    : "contains"
    club_books          ||--o{ club_list_books    : "listed in"
    club_members        ||--o{ club_lists         : "owns"
...
| `schema_migrations` | `version` | Ordered ledger for guarded, one-time application migrations. |
