...
schema on 2026-06-27 (40 physical tables; the 6 `mail_message_fts*` tables are one FTS5 virtual
...
5. ~~**Missing FK-column indexes.**~~ **Done.** Added `idx_attendance_member`,
   `idx_reading_statuses_member`, `idx_member_contacts_member`, `idx_email_tracking_member`,
   `idx_email_tracking_contact`. Also removed `identity_claims` (a dead write-only staging table) and
   added an `sms` member-handle surface.
